...
const IntroduceChairman = () => {
  return (
    <>
      <div className={`w-full max-w-1200 pb-120 pt-80`}>
        <div className={`flex items-start justify-center gap-26`}>
          <div
            className={`mt-34 flex flex-col items-start justify-start gap-40`}
          >
            <p className={`text-regular48 line-clamp-3 text-grey-900`}>
              지금 전세계는
              <br />
              <span className={`text-bold48`}>디지털 혁명</span>이 이루어지
              <br /> 고 있습니다.
            </p>

            <p className={`text-regular18 line-clamp-4 text-grey-500`}>
              삶에 필요한 모든 것들이 디지털 프로그램을 통해 빠르게 공급되
              <br />고 있습니다. 이러한 시대가 도래한 이유를 선교적인 시각에서
              보
              <br /> 면 바로 마지막 때가 가까워졌기 때문입니다. 그래서
              하나님께서
              <br /> 는 복음이 빨리 전해지는 길을 만들어 주신 것입니다.
            </p>

            <p className={`text-regular24 line-clamp-2 text-grey-900`}>
              온누리선교재단 이사장
          ...
          <img
            className={`h-708 w-733`}
            src={`${import.meta.env.VITE_PUBLIC_URL}images/introduce/chairman.png`}
            width={733}
            height={708}
            alt=''
          />
      ...
      <div
        className={`flex w-full max-w-1200 flex-col items-start justify-start gap-40 pb-160 pt-120`}
      >
        <div>
          <p className={`text-bold40 line-clamp-1 text-grey-900`}>
            안녕하세요 CGN 이사장 이재훈 목사입니다.
          </p>
          <p className={`text-bold18 mt-12 line-clamp-1 text-grey-900`}>

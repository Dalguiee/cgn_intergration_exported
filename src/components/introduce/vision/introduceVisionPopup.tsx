...
const IntroduceVisionPopup = ({ popupOpen, setPopupOpen }) => {
  ...
  return (
    <>
      ...
      ></div>
      <div
        className={`${popupOpen ? `` : `hidden`} absolute left-[50%] top-175 z-20 flex h-fit w-full max-w-960 translate-x-[-50%] transform flex-col items-center justify-start gap-60 bg-white-solid px-40 pb-60 pt-24 max-lg:top-10 max-lg:w-[calc(100%-20px)] max-lg:px-16`}
      >
        ...
        <div className={`w-full`}>
          <div className={`mb-24`}>
            <p className={`text-bold24 max-lg:text-bold16 mb-4 text-grey-900`}>
              로고
            </p>
          ...
          <div>
            <p className={`text-regular16 max-lg:text-regular14 text-grey-500`}>
              CGN의 로고는 콜론(:)을 모티브로 합니다. 이는 진리 안에서 무엇이든
              될 수 있는 CGN의 자유로운 확장성을 의미합니다. 콜론은 성경의 장과
              절을, 대본에서 등장인물과 대사 사이를 구분합니다. 콜론 뒤의 대사로
              등장인물의 캐릭터와 정체성이 드러나는 것처럼 CGN 뒤에 붙는 말들로
              CGN의 정체성이 만들어집니다. CGN은 새로운 챕터에서 다양한 사역과
              도전을 써 내려갈 것입니다. 본 로고는 CGN의 아이덴티티를 시각적으로
              전달하는 가장 핵심적인 요소입니다. 로고의 형태나 색상을 변형하여
              사용하지 않도록 세심한 주의를 요하며 어떤 상황에서도 승인된 CI
              파일만 사용해야 합니다.
            </p>
          </div>
        </div>
        ...
        <div className={`w-full`}>
          ...
          <div>
            ...
            <p className={`text-regular16 max-lg:text-regular14 text-grey-500`}>
              ...
              담대하게 복음을 전합니다.-
              <br />
              CGN의 색상 표현은 매뉴얼에 명시된 Pantone Color를 표준으로 합니다.
              전용 색상은 CGN의 이미지를 전달하는 중요한 수단이므로 지정된
              <br />
